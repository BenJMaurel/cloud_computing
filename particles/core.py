...
class SMC(object):
    ...
    def reweight_particles(self):
        #logG = self.fk.logG(self.t, self.scatttered_Xp, self.scattered_X)
        #self.wgts = self.wgts.add(logG)
    ...
    def resample_move(self):
        ...
        if self.rs_flag:  # if resampling
            ...
            self.Xp = self.X
        def array_split(N, arr, num_splits, axis=0):
            length = N
            
            split_size = length // num_splits
            remainder = length % num_splits
            
            split_lengths = [split_size] * num_splits
        ...
        if self.mpi_comm:
            ...
            partitioned_inputs = array_split(self.Xp.N, self.Xp, self._size)
            reqs1 = []
            reqs2 = []
            for i in range(1, self._size):
                dict1 = pickle.dumps(copy.deepcopy(partitioned_inputs[i].dict_fields))
                dict2 = [partitioned_inputs[i].shared['exponents'], partitioned_inputs[i].shared['chol_cov']]
                
                req1 = self._comm.isend(dict1, dest=i, tag=123)
                req2 = self._comm.isend(dict2, dest=i, tag=124)
                
                reqs1.append(req1)
                reqs2.append(req2)

            local_result = self.fk.M(self.t, partitioned_inputs[0])

            for req1, req2 in zip(reqs1, reqs2):
                req1.wait()
                req2.wait()
            ...
            reqs2 = []
            for i in range(1, self._size):
                req1 = self._comm.irecv( source=i, tag=234)
                dict1 = pickle.loads(dict1)
                local_result.add_from_dict(dict1)
    ...
    def __next__(self):
        ...
        if self.t == 0:
            ...
            if self.qmc:
                ...
                self.resample_move()
        # partitioned_Xp = np.array_split(self.Xp, self._size)
        # partitioned_X = np.array_split(self.X, self._size)
        # self.scattered_X = self._comm.scatter(partitioned_X, root=0)
        # self.scattered_Xp = self._comm.scatter(partitioned_Xp, root=0)
