...
class MPIBoostrap(particles.FeynmanKac):
    def __init__(self, ssm=None, data=None, mpi_comm = None):
    ...
    def M(self, t, xp):
        partitioned_inputs = np.array_split(xp, self._size)
        scattered_inputs = self._comm.scatter(partitioned_inputs, root=0)
        local_result = self.ssm.PX(t, scattered_inputs).rvs(size=scattered_inputs.shape[0])
        gathered_outputs = self._comm.allgather(local_result)
        results = np.concatenate(gathered_outputs)
        return results
