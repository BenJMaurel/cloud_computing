...
class ThetaParticles(object):
    ...
    * a common attribute (shared among all particles).
    ...
    def add(self, theta_part):
        for field in self.dict_fields.keys():
            self.__dict__[field] = np.concatenate((self.dict_fields[field], theta_part.dict_fields[field]))

    def add_from_dict(self, dict1 = {}, dict2 = None):
        for field in self.dict_fields.keys():
            self.__dict__[field] = np.concatenate((self.dict_fields[field], dict1[field]))
        if dict2:
            for field in self.shared.keys():
                self.shared[field] = dict2[field]
...
class ArrayRandomWalk(ArrayMetropolis):
    ...
    def calibrate(self, W, x):
        ...
        try:
            ...
            x.shared['chol_cov'] = scale * np.zeros(cov.shape)
